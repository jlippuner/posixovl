...
static unsigned int single_threaded, hardlink_with_copy;
...
inline static bool supports_owners(const char *path, uid_t uid,
    gid_t gid, bool restore)
{
  return false;
}

inline static bool supports_permissions(const char *path)
{
  return false;
}
...
{
  fprintf(stderr,
      "Usage: %s [-H] [-S source] mountpoint [-- fuseoptions]\n", p);
  exit(EXIT_FAILURE);
}
...
int main(int argc, char **argv)
{
  ...
  while ((c = getopt(argc, argv, "1HS:")) > 0) {
    switch (c) {
    case '1':
